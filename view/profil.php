...
<head>
	<style>
		...
		.profil a{
			color: <?= $infoparametre['colorlien'] ?> !important;
		}
		.profil a:hover{
			text-decoration: <?= $infoparametre['colordecorationlien'] ?> underline !important;
		}
		form .submit{
		...
		<div class="topicality">
			<table cellspacing="5">
				<tr>
					...
					<td>
						<form method="POST" action="index.php?action=profil&id=<?= $userinfo['id']; ?>">
				...
				while($data = $reqtopicality->fetch()){
					...
					<tr>
						...
						<td>
							<p class="title">
								<a href="index.php?action=visited&name=<?= $data['name'] ?>" class="pseudo"><?= $data['name'] ?></a><a href="index.php?action=billet&idbillet=<?= $data['id'] ?>&id=<?= $userinfo['id'] ?>" style="color: #000; text-decoration: red underline;"> à <?= $data['heure']; ?> le <?= $data['Jour']; ?> à publié :</a>
							</p> 
							<p class="message">
								<?= $data['message'] ?>
							</p>
							...
							<form method="POST" action="index.php?action=profil&id=<?= $userinfo['id'] ?>" style="display: flex; justify-content: center;">
